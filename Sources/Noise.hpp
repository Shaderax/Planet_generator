...
static const uint8_t Source[256] = {
...
class Noise
{
	...
	public:
		...
		void Randomize( int32_t seed )
		{
			...
			{*/
			for (int i = 0; i < 256; i++)
				_random[i + 256] = _random[i] = Source[i];
			//}
		}
...
double CosineInterpolate(float a, float b, float t)
{
	float c = (1 - cos(t * M_PI)) * .5;
